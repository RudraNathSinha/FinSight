...
# Monorepo root = FinSightApplication/
...
PART3_DIR = os.path.join(FINSIGHT_ROOT, "part3_sector_industry")
PART3_URL = os.environ.get("FINSIGHT_PART3_URL", "/sector")  # same-origin
...
def _load_iso_maps():
    ...
    try:
        ...
        print(f"[FinSight] ISO maps load skipped: {e}")
_load_iso_maps()
...
# Landing assets at root (index.html references them relatively)
@app.get("/styles.css")
def landing_css():
...
@app.get("/app.js")
def landing_app_js():
    p = os.path.join(LANDING_DIR, "app.js")
    if os.path.exists(p):
        return FileResponse(p, media_type="application/javascript")
    return JSONResponse({"error": "missing"}, status_code=404)
...
# ---------------------------------------------------------------------------
# Part 3 (Sector Intelligence) live APIs — Flask-compatible shapes
# Frontend expects: history = [{Date, Open, High, Low, Close, Volume}, ...]
#                   quote/profile = flat yfinance info keys (currentPrice, trailingPE, ...)
#                   financials = {income_stmt, balance_sheet, cashflow, ...} as record lists
# ---------------------------------------------------------------------------
_P3_CACHE_QUOTE = {}
_P3_CACHE_HIST = {}
_P3_CACHE_TTL = 60  # seconds

def _p3_cache_get(store, key):
    try:
        item = store.get(key)
        if not item:
            return None
        ts, val = item
        if time.time() - ts > _P3_CACHE_TTL:
            return None
        return val
    except Exception:
        return None

def _p3_cache_set(store, key, val):
    try:
...
def _p3_json_safe(obj):
    if obj is None:
        return None
    if isinstance(obj, dict):
        return {k: _p3_json_safe(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [_p3_json_safe(v) for v in obj]
    ...
    if hasattr(obj, "item"):
        try:
            return _p3_json_safe(obj.item())
        except Exception:
            return None
    try:
        if YF_AVAILABLE and pd.isna(obj):
            return None
    except Exception:
        pass
    return obj
...
def _p3_df_records(df):
    if df is None or (hasattr(df, "empty") and df.empty):
        return None
    df = df.copy()
    df = df.replace([float("inf"), float("-inf")], pd.NA)
    # columns may be Timestamps when statements are transposed
    records = df.where(pd.notnull(df), None).to_dict(orient="records")
    return _p3_json_safe(records)


def _p3_stmt_records(df):
    """yfinance statement: index=metric, columns=periods → list of row dicts with index key."""
    if df is None or (hasattr(df, "empty") and df.empty):
        return None
    out = []
    try:
        dft = df.copy()
        dft.columns = [str(c)[:10] if hasattr(c, "strftime") else str(c) for c in dft.columns]
        dft = dft.reset_index()
        col0 = dft.columns[0]
        dft = dft.rename(columns={col0: "index"})
        dft = dft.replace([float("inf"), float("-inf")], pd.NA)
        out = dft.where(pd.notnull(dft), None).to_dict(orient="records")
    except Exception:
        return None
    return _p3_json_safe(out)


@app.get("/api/live/quote/{symbol}")
def part3_live_quote(symbol: str):
    if not YF_AVAILABLE:
        return JSONResponse({"error": "yfinance not installed", "code": "UPSTREAM", "symbol": symbol}, status_code=502)
    key = (symbol or "").upper().strip()
    if not key:
        return JSONResponse({"error": "Symbol required", "code": "EMPTY_SYMBOL"}, status_code=400)
    try:
        t = yf.Ticker(key)
        info = {}
        try:
            ...
            info = {}
        if not info or (
            not info.get("shortName") and not info.get("longName")
            ...
            try:
                ...
                pass
            return JSONResponse({"error": f"No quote data for {key}", "code": "NO_DATA", "symbol": key}, status_code=404)
        keys = [
        ...
        return JSONResponse({"error": str(e), "code": "UPSTREAM", "symbol": key}, status_code=502)


@app.get("/api/live/history/{symbol}")
def part3_live_history(symbol: str, period: str = "1y", interval: str = "1d"):
    """Return list of OHLCV row dicts (Part 3 Chart.js expects this)."""
    if not YF_AVAILABLE:
        return JSONResponse({"error": "yfinance not installed", "code": "UPSTREAM", "symbol": symbol}, status_code=502)
    key = (symbol or "").upper().strip()
    if not key:
    ...
    cached = _p3_cache_get(_P3_CACHE_HIST, ckey)
    if cached is not None:
        return cached
    try:
        t = yf.Ticker(key)
        hist = t.history(period=period or "1y", interval=interval or "1d")
        if hist is None or hist.empty:
            return JSONResponse({"error": f"No history for {key}", "code": "NO_DATA", "symbol": key}, status_code=404)
        hist = hist.reset_index()
        for col in hist.columns:
            dtype = str(hist[col].dtype).lower()
            if "datetime" in dtype or "date" in dtype:
                hist[col] = hist[col].astype(str)
        # Normalize column names Part 3 reads: Date/Datetime, Close, Open, High, Low, Volume
        rename = {}
        for c in hist.columns:
            cl = str(c)
            if cl.lower() in ("date", "datetime", "index"):
                rename[c] = "Date" if "date" in cl.lower() or cl.lower() == "index" else "Datetime"
            elif cl.lower() == "close":
                rename[c] = "Close"
            elif cl.lower() == "open":
                rename[c] = "Open"
        ...
        records = _p3_df_records(hist)
        if not records:
            return JSONResponse({"error": f"No history for {key}", "code": "NO_DATA", "symbol": key}, status_code=404)
        _p3_cache_set(_P3_CACHE_HIST, f"{key}|{period}|{interval}", records)
        return records
    except Exception as e:
        return JSONResponse({"error": str(e), "code": "UPSTREAM", "symbol": key}, status_code=502)
...
def part3_live_financials(symbol: str):
    ...
    try:
        t = yf.Ticker(key)
        income = _p3_stmt_records(getattr(t, "financials", None))
        income_q = _p3_stmt_records(getattr(t, "quarterly_financials", None))
        balance = _p3_stmt_records(getattr(t, "balance_sheet", None))
        balance_q = _p3_stmt_records(getattr(t, "quarterly_balance_sheet", None))
        cashflow = _p3_stmt_records(getattr(t, "cashflow", None))
        cashflow_q = _p3_stmt_records(getattr(t, "quarterly_cashflow", None))
        payload = {
            "symbol": key,
            "income_stmt": income,
            "income_statement": income,
            "quarterly_income_stmt": income_q,
            "income_stmt_quarterly": income_q,
            "balance_sheet": balance,
...
def part3_live_dividends(symbol: str):
    ...
    try:
        ...
        if divs is None or (hasattr(divs, "empty") and divs.empty):
            return {"symbol": key, "dividends": []}
        out = [{"date": str(idx.date()) if hasattr(idx, "date") else str(idx)[:10], "amount": float(val)} for idx, val in divs.items()]
        return {"symbol": key, "dividends": out[-80:]}
    except Exception as e:
        return {"symbol": key, "dividends": [], "error": str(e)}


@app.get("/api/stocks/{symbol}")
def part3_stock_profile(symbol: str):
    """Profile alias used by Part 3 — same flat quote shape."""
    return part3_live_quote(symbol)
...
def country_ranking_app():
    for path in (
        os.path.join(PART1_DIR, "index.html"),
        os.path.join(FRONTEND, "country-ranking", "index.html"),
    ):
        if os.path.exists(path):
...
# Equity SPA fallbacks (before static mounts)
...
# ---------------------------------------------------------------------------
# Static mounts — single origin, no cross-port links
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  FinSight Application (unified — one port)")
