...
// Part1 handoff — Country Ranking "Explore listings" deep-link
(function () {
  try {
    const qs = new URLSearchParams(location.search);
    // Prefer human-readable name for openCountry(); ISO3 is secondary
    const country = qs.get('country_name') || qs.get('country');
    const exchange = qs.get('exchange');
    if (country) window.__FINSIGHT_PENDING_COUNTRY = country;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    setTimeout(function () {
      try {
        if (typeof openCountry === 'function') {
          openCountry(c);
        } else if (typeof navigate === 'function') {
          navigate('country-page', { country: c });
        }
      } catch (e) {
        try {
