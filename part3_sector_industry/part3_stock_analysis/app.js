...
async function loadFinancialsLive(sym) {
  ...
  try {
    ...
    body.innerHTML = `<div class="card p-6 text-center text-amber-600 text-sm">Financials unavailable: ${e.message}<br><span class="text-xs text-slate-400">Ensure FinSight is running on port 8000</span></div>`;
  }
}
...
document.getElementById('sidebarCloseBtn')?.addEventListener('click', () => {
    ...
    console.error(err);
    document.getElementById('loader').innerHTML = `<div class="text-center max-w-md px-4"><p class="text-red-500 font-semibold">Failed to load data.</p><p class="text-sm text-slate-500 mt-2">${(err && err.message) || ''}</p><p class="text-xs text-slate-400 mt-2">Start the backend: <code>python -m uvicorn backend.main:app --port 8000</code> then open <code>http://localhost:8000/part3_stock_analysis/</code></p></div>`;
  }
})();
