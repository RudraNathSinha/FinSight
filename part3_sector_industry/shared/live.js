...
(function (global) {
  ...
  function classifyError(status, body, url) {
    ...
    if (status === 0) {
      return {
        kind: 'backend_down',
        message: 'Cannot reach FinSight backend. Start: python -m uvicorn backend.main:app --port 8000 (from part2_global_equity)',
        status,
        code: 'BACKEND_DOWN',
        url,
      };
  ...
  async function get(path) {
    ...
    try {
      ...
      const err = new Error('Cannot reach FinSight API at ' + (BASE || (typeof location !== 'undefined' ? location.origin : 'localhost:8000')) + ' — run: python -m uvicorn backend.main:app --port 8000');
  ...
  async function downloadPdf(html, filename) {
    ...
    try {
      ...
      const err = new Error('PDF service unreachable — is FinSight running on port 8000?');
      err.code = 'BACKEND_DOWN';
      throw err;
    }
    if (!res.ok) {
  ...
  function errorBannerHtml(meta, opts) {
    ...
    const tip =
      kind === 'backend_down'
        ? 'Start FinSight: <code class="text-xs bg-slate-100 dark:bg-slate-800 px-1 rounded">python -m uvicorn backend.main:app --port 8000</code> then refresh.'
        : kind === 'no_data'
  ...
  function setBackendChip(ok, detail) {
    ...
    if (ok) {
      ...
      el.title = detail || 'Start: python -m uvicorn backend.main:app --port 8000';
    }
  }
